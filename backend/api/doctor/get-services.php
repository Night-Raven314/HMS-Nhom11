  ...
  if ($data) {
    ...
    $sql = "WITH
      ...
      SELECT
        ...
        itm.item_unit,
        CASE
          WHEN (fac.amount IS NULL AND fac.end_datetime IS NULL) THEN timestampdiff(day, CONVERT_TZ(STR_TO_DATE(fac.start_datetime, '%Y-%m-%dT%H:%i:%s'), '+00:00', 'Asia/Ho_Chi_Minh'), CURRENT_TIMESTAMP())
          ...
          WHEN timestampdiff(day, CONVERT_TZ(STR_TO_DATE(fac.start_datetime, '%Y-%m-%dT%H:%i:%s'), '+00:00', 'Asia/Ho_Chi_Minh'), CURRENT_TIMESTAMP()) = 0 THEN 1
